...
{
    public class PessoaPadraoERPOut_v1_1_Contexto:Contexto
    {
        ...
        public string GetXMLFromGera()
        {
            try
            {
                ...
                var export = conexion_SOAP.ExportarDados(Token, "PessoaPadraoERP", "1.1");
        ...
        public PessoaPadraoERPOut_v1_1[] GetPessoaFromXML(string xml, bool grabarTicket)
        {
            try
            {
                OnLog(new ObjLog() { Tipo = 4, Ticket = Ticket, CodigoDocumento = 0, Tabla = "PessoaPadraoERP V1.1", Mensaje = "Deserializando..." });
                XmlSerializer deserializer = new XmlSerializer(typeof(PessoaPadraoERPOut_v1_1[]), new XmlRootAttribute("ArrayOfPessoaPadraoERPOut_v1_1"));
                PessoaPadraoERPOut_v1_1[] personas;
                OnLog(new ObjLog() { Tipo = 5, Ticket = Ticket, CodigoDocumento = 0, Tabla = "PessoaPadraoERP V1.1", Mensaje = "Convertiendo a objetos Personas" });
                TextReader reader = new StringReader(xml);
                personas = (PessoaPadraoERPOut_v1_1[])deserializer.Deserialize(reader);
                if (grabarTicket)
            ...
            {
                OnLog(new ObjLog() { Tipo = -2, CodigoDocumento = Ticket, Tabla = "PessoaPadraoERP V1.1", Mensaje = "Excepcion en la serializacion de PessoaPadraoERP: " + ex.Message });
                throw new Exception("Excepcion en la serializacion de Personas V1.1: " + ex.Message);
            }
        }


        public string GrabarPersonas(PessoaPadraoERPOut_v1_1[] personas)
        {
            ...
            using (var conn = new SqlConnection(cnn))
            {
                foreach (var persona in personas)
                {
                    try
                    {
                        ...
                        OnLog(new ObjLog() { Tipo = 7, Ticket = Ticket, CodigoDocumento = persona.CodigoDocumento, Tabla = "PessoaPadraoERP V1.1", Mensaje = "Grabacion de PessoaPadraoERP, tipo: " + insertUpdate + " Código: " + persona.Codigo });
                    ...
                    catch (Exception ex)
                    {
                        OnLog(new ObjLog()
                        {
                            Tipo = -3,
                            Ticket = Ticket,
                            CodigoDocumento = persona.CodigoDocumento,
                            Tabla = "Pessoa",
        ...
        public bool CerrarTicket(PessoaPadraoERPOut_v1_1[] personas)
        {
            ...
            if (HayQueCerrarTicket == "NO")
            {
                OnLog(new ObjLog()
                {
                    ...
                    Ticket = Ticket,
                    CodigoDocumento = 0,
                    Tabla = "PessoaPadraoERPOut_v1.1",
                    Mensaje = "No se esta cerando el Ticket PessoaPadraoERPOut_v1"
                });
                return true;
            }

            OnLog(new ObjLog() { Tipo = 99, Ticket = Ticket, CodigoDocumento = 0, Tabla = "PessoaPadraoERP V1.1", Mensaje = "Cerrando ticket " });
            DateTime fecha;
            ...
            try
            {
                var cierre = conexion_SOAP.AdicionarConfirmacaoTicket(Token, "PessoaPadraoERP", "1.1", Ticket, documentos, out fecha, out errores);
                OnLog(new ObjLog() { Tipo = 100, Ticket = Ticket, CodigoDocumento = 0, Tabla = "PessoaPadraoERP V1.1", Mensaje = "Tickets cerrado correctamente." });
                return true;

            }
            catch (Exception ex)
            {
                OnLog(new ObjLog() { Tipo = -4, Ticket = Ticket, CodigoDocumento = 0, Tabla = "PessoaPadraoERP V1.1", Mensaje = "Error cerrando ticket PessoaPadraoERP: " + ex.Message });
                return false;
            }
        }
